...
/** Validated scalar values used by HLS playlists.
  *
  * The grammar comes from [[https://www.rfc-editor.org/rfc/rfc8216#section-4.2
  * RFC 8216 section 4.2]]. Opaque types prevent accidentally exchanging values
  * that have the same JVM representation but different protocol meanings.
  */
object ValueTypes:
  ...
  object Duration:
    ...
    def parse(value: String): Either[String, Duration] =
      Try(BigDecimal(value)).toEither.left.map(_ => s"invalid decimal duration: $value").flatMap(from)
    def unsafe(value: BigDecimal): Duration = from(value).fold(message => throw IllegalArgumentException(message), identity)
  ...
  object Bandwidth:
    ...
    def unsafe(value: Long): Bandwidth = from(value).fold(message => throw IllegalArgumentException(message), identity)
  ...
  object MediaSequence:
    ...
    def parse(value: String): Either[String, MediaSequence] =
      value.toLongOption.toRight(s"invalid decimal integer: $value").flatMap(from)
    def unsafe(value: Long): MediaSequence = from(value).fold(message => throw IllegalArgumentException(message), identity)
  ...
  object PlaylistUri:
    def parse(value: String): Either[String, PlaylistUri] =
      Try(URI.create(value)).toEither.left.map(_ => s"invalid URI: $value")
        .filterOrElse(uri => !uri.toString.exists(c => c == '\r' || c == '\n'), "URI contains a line break")
    def unsafe(value: String): PlaylistUri = parse(value).fold(message => throw IllegalArgumentException(message), identity)
...
object Resolution:
  def from(width: Int, height: Int): Either[String, Resolution] =
    Either.cond(width > 0 && height > 0, Resolution(width, height), s"invalid resolution: ${width}x$height")
