...
/** Production-shaped synchronous HTTP client for HLS playlists.
  *
  * The client applies bounded request timeouts, bounded response bodies,
  * explicit UTF-8 decoding, gzip decoding, redirect policy, conditional GET,
  * parsing, semantic validation, and URI resolution. Network calls are
  * synchronous by design; callers can place them in their preferred effect or
  * execution abstraction without this library choosing one.
  */
final class HlsClient private (
  ...
  private def request(uri: URI, previous: Option[PlaylistSnapshot]): Either[ClientError, Response] =
    val builder = HttpRequest.newBuilder(uri).timeout(requestTimeout)
      .header("Accept", "application/vnd.apple.mpegurl, application/x-mpegURL, audio/mpegurl, */*;q=0.1")
      .header("Accept-Encoding", "gzip")
      .header("User-Agent", "learn-hls/0.1")
      .GET()
    previous.flatMap(_.entityTag).foreach(builder.header("If-None-Match", _))
    if previous.flatMap(_.entityTag).isEmpty then previous.flatMap(_.lastModified).foreach(builder.header("If-Modified-Since", _))
    Try(http.send(builder.build(), HttpResponse.BodyHandlers.ofByteArray())).toEither
      .left.map(ClientError.Transport(uri, _)).flatMap: response =>
        response.statusCode() match
          case 304 if previous.nonEmpty => Right(Response.NotModified)
          case status if status >= 200 && status < 300 =>
            Either.cond(response.body().length <= maximumBytes,
  ...
  private def decode(requested: URI, effective: URI, headers: java.net.http.HttpHeaders, body: Array[Byte]): Either[ClientError, PlaylistSnapshot] =
    for
      ...
      _ <- Either.cond(decompressed.length <= maximumBytes, (), ClientError.BodyTooLarge(effective, maximumBytes))
    ...
    yield PlaylistSnapshot(requested, effective, PlaylistResolver.resolve(parsed, effective),
      firstHeader(headers, "ETag"), firstHeader(headers, "Last-Modified"), clock())
  ...
  private def utf8(bytes: Array[Byte]): Option[String] =
    Try(StandardCharsets.UTF_8.newDecoder().onMalformedInput(CodingErrorAction.REPORT)
      .onUnmappableCharacter(CodingErrorAction.REPORT).decode(java.nio.ByteBuffer.wrap(bytes)).toString).toOption
  ...
  private def gunzip(bytes: Array[Byte]): Either[Throwable, Array[Byte]] = Try:
    Using.resource(GZIPInputStream(ByteArrayInputStream(bytes))): input =>
      ...
      while read >= 0 do
        total += read
        if total > maximumBytes then throw IllegalArgumentException("decompressed playlist exceeds maximum size")
...
object HlsClient:
  def create(
    ...
    val http = HttpClient.newBuilder().connectTimeout(connectTimeout)
      .followRedirects(HttpClient.Redirect.NORMAL).build()
    HlsClient(http, requestTimeout, maximumBytes, () => Instant.now())

  private[client] def testing(http: HttpClient, maximumBytes: Int, clock: () => Instant): HlsClient =
