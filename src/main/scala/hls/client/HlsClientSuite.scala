...
final class HlsClientSuite extends munit.FunSuite:
  ...
  test("load gzip, preserve metadata, and resolve every URI"):
    withServer: (server, base) =>
      server.createContext("/live/index.m3u8", exchange =>
      ...
      assertEquals(media.segments.head.initializationMap.get.uri.toString, base.resolve("/live/init.mp4").toString)
      media.segments.head.encryption match
        case Encryption.Aes128(uri, _) => assertEquals(uri.toString, base.resolve("/keys/key.bin").toString)
  ...
  test("reject oversized and malformed UTF-8 bodies"):
    withServer: (server, base) =>
      ...
      server.createContext("/utf8.m3u8", exchange =>
      ...
      assert(client.load(base.resolve("/large.m3u8")).left.toOption.exists(_.isInstanceOf[ClientError.BodyTooLarge]))
      assert(client.load(base.resolve("/utf8.m3u8")).left.toOption.exists(_.isInstanceOf[ClientError.InvalidUtf8]))
