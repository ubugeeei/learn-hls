...
final class LivePlaylistSuite extends munit.FunSuite:
  private def segment(index: Int) =
    MediaSegment(PlaylistUri.unsafe(s"$index.ts"), Duration.unsafe(2))
  ...
  test("builder infers the compatibility version for fMP4"):
    val fmp4 =
      segment(0).copy(initializationMap = Some(InitializationMap(PlaylistUri.unsafe("init.mp4"))))
  ...
  test("builder accumulates semantic validation"):
    val result = MediaPlaylistBuilder
      .create(2)
      .toOption
      .get
      .add(segment(0).copy(duration = Duration.unsafe(3)))
      .asVod
      .build
