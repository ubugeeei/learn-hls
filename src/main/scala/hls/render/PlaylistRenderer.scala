...
object PlaylistRenderer:
  ...
  def renderMedia(playlist: MediaPlaylist): String =
    val header = Vector("#EXTM3U") ++
      ...
      Option.when(playlist.mediaSequence.value != 0)(s"#EXT-X-MEDIA-SEQUENCE:${playlist.mediaSequence.value}") ++
      Option.when(playlist.discontinuitySequence != 0)(s"#EXT-X-DISCONTINUITY-SEQUENCE:${playlist.discontinuitySequence}") ++
    ...
    val body = playlist.segments.flatMap: segment =>
      ...
      val mapLine = Option.when(segment.initializationMap != initMap)(segment.initializationMap.map(renderMap).getOrElse(""))
  ...
  def renderMultivariant(playlist: MultivariantPlaylist): String =
    ...
    val renditionLines = playlist.renditions.map: rendition =>
      val attributes = Vector(
        ...
        Some("GROUP-ID" -> quote(rendition.groupId)), Some("NAME" -> quote(rendition.name)),
        rendition.uri.map(uri => "URI" -> quote(uri.toString)), rendition.language.map("LANGUAGE" -> quote(_)),
        Option.when(rendition.default)("DEFAULT" -> "YES"), Option.when(rendition.autoselect)("AUTOSELECT" -> "YES"),
        Option.when(rendition.forced)("FORCED" -> "YES"),
        Option.when(rendition.characteristics.nonEmpty)("CHARACTERISTICS" -> quote(rendition.characteristics.mkString(",")))
      ).flatten
    ...
    val variantLines = playlist.variants.flatMap: variant =>
      val attributes = Vector(
        ...
        variant.audioGroup.map("AUDIO" -> quote(_)), variant.videoGroup.map("VIDEO" -> quote(_)),
        variant.subtitlesGroup.map("SUBTITLES" -> quote(_)), variant.closedCaptionsGroup.map("CLOSED-CAPTIONS" -> quote(_))
      ).flatten
      Vector(s"#EXT-X-STREAM-INF:${attributes.map((k, v) => s"$k=$v").mkString(",")}", variant.uri.toString)
  ...
  private def renderKey(encryption: Encryption): String = encryption match
    ...
    case Encryption.SampleAes(uri, keyFormat, versions, iv) =>
      val optional = Vector(keyFormat.map(v => s"KEYFORMAT=${quote(v)}"), versions.map(v => s"KEYFORMATVERSIONS=${quote(v)}"), iv.map(v => s"IV=$v")).flatten
      (Vector(s"#EXT-X-KEY:METHOD=SAMPLE-AES", s"URI=${quote(uri.toString)}") ++ optional).mkString(",")

  private def renderMap(map: InitializationMap): String =
    s"#EXT-X-MAP:URI=${quote(map.uri.toString)}${map.byteRange.fold("")(r => s",BYTERANGE=${quote(s"${r.length}@${r.offset}")}")}" 
